	...
	<account title="فرم ها">
		<div class="table-responsive text-center bg-white rounded-3 shadow-sm px-4">
			...
			<table class="table table-striped" v-if="resource.data?.length">
				...
				<template v-for="(form, index) in resource.data" :key="index">
					<tr class="table-row">
						<td>{{ form.id }}</td>
						<td>{{ form.title }}</td>
						<td>
							<button class="btn btn-sm btn-outline-danger mx-1 mt-1" data-bs-toggle="modal" data-bs-target="#delete-modal" @click="selectedItem = form.id">
								<i class="bi bi-trash-fill"></i>
							</button>
							<button class="btn btn-sm btn-outline-warning mx-1 mt-1" data-bs-toggle="modal" data-bs-target="#update-modal" @click="editItem(form)">
								<i class="bi bi-pencil-fill"></i>
							</button>
							<button class="btn btn-sm btn-outline-primary mx-1 mt-1" @click="$router.push('/account/forms/' + form.id)" v-if="form.queries.length > 0">
								<i class="bi bi-eye"></i>
							</button>
						</td>
					</tr>
					<tr class="table-row" v-if="form.queries.length > 0">
						<td colspan="3">
							<table class="table">
								<thead class="table-head">
								<tr>
